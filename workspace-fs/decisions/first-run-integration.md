...
- When llms-furl has files it can update for integration, in TTY request
  permission with an **opensrc-style list + y/n prompt**.
...
## What llms-furl may update

- `.gitignore`: add `llms-furl/` to ignore
- `tsconfig.json`: add `llms-furl` to `exclude` (if it cannot be updated safely,
  do **not** auto-update and fall back to a manual update hint)
- `AGENTS.md`: add/update a marker-wrapped section
  (`<!-- llms-furl:start --> ... <!-- llms-furl:end -->`)
...
- Persist initial integration consent in the llms-furl state file
  `llms-furl/.llms-furl.json` (e.g. `integration.consent` and
  `integration.applied`).
